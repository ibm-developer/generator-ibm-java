...
describe('java generator : basic integration test', function () {
  ...
  describe('Generates a basic web project (no bluemix)', function () {
    ...
    it('with gradle build system', function (done) {
      ...
      helpers.run(path.join( __dirname, '../../generators/app'))
        .withOptions(options)
        .withPrompts({extName : 'ext:bluemix', buildType : 'gradle', createType: 'basicweb', services: ['none'], appName: APPNAME})
    ...
    it('with maven build system', function (done) {
      ...
      helpers.run(path.join( __dirname, '../../generators/app'))
        .withOptions(options)
        .withPrompts({extName : 'ext:bluemix', buildType : 'maven', createType: 'basicweb', appName: APPNAME })
      .toPromise().then(function() {
        try {
          options.assert(APPNAME, APPNAME, false, false);
  ...
  describe('Generates a basic web project (bluemix)', function () {
    ...
    it('with cloudant', function (done) {
      ...
      helpers.run(path.join( __dirname, '../../generators/app'))
        .withOptions(options)
        .withPrompts({extName : 'ext:bluemix', buildType : 'maven', createType: 'basicweb', services : ['cloudant'], appName : 'bxName' })
      .toPromise().then(function() {
        try {
    ...
    it('with objectStorage', function (done) {
      ...
      helpers.run(path.join( __dirname, '../../generators/app'))
        .withOptions(options)
        .withPrompts({extName : 'ext:bluemix', buildType : 'maven', createType: 'basicweb', services : ['objectStorage'], appName : 'bxName' })
      .toPromise().then(function() {
        try {
    ...
    it('with cloudant and objectStorage', function (done) {
      ...
      helpers.run(path.join( __dirname, '../../generators/app'))
        .withOptions(options)
        .withPrompts({extName : 'ext:bluemix', buildType : 'maven', createType: 'basicweb', services : ['objectStorage', 'cloudant'], appName : 'bxName' })
      .toPromise().then(function() {
        try {
